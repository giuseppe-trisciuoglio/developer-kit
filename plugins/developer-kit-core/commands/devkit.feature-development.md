...
3. Read task details (description, acceptance criteria)
4. Implement the task following acceptance criteria
5. Mark task as completed in the task list file
6. Report completion
...
```
If $ARGUMENTS starts with "Task:" (case-insensitive):
...
Task: API endpoint implementation     → Task mode (case-insensitive)
...
1. **Extract task name**: Remove "Task:" prefix, trim whitespace
...
When Feature Mode is detected (no Task: prefix):
...
## Task Mode Flow (When "Task:" prefix detected)
...
1. Parse $ARGUMENTS to extract task name:
   - Remove "Task:" prefix (case-insensitive)
   - Trim whitespace
...
**For Task Mode (when "Task:" prefix detected):**
```
[ ] T-1: Task Identification
[ ] T-2: Dependency Check
[ ] T-3: Implementation
